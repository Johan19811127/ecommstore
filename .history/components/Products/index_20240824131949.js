      ...
      <div className="grid lg:grid-cols-3 gap-2 align-items-center">
        {products.map((product) => (
          <div key={product._id} className="container m-1 border rounded-xl">
            <div className="container bg-slate-100 flex aspect-square items-center content-center rounded-t-xl border-b-slate-200">
              {product.images && product.images[1] && (
                <Image src={product.images[0]} alt={product.name} width="400" height="400" className="border-b-slate-200 rounded-t-xl"/>
              )}
            </div>
            <div className="container px-3 py-2">
              <p className="text-lg font-semibold  font-sans antialiased uppercase">{product.name}</p>
