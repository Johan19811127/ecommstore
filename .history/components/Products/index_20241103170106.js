...
import ProductsPrice from './price';
...
export default function Products() {
  ...
  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-4 align-items-center">
      ...
      <div key={product._id} className="container m-1 border-b border-slate-200 shadow-md rounded-xl">
        <Link href={`/products/${product._id}`}>
          ...
          <div className="container px-5 py-5">
            <p className="text-md lg:text-lg font-bold antialiased lg:text-center leading-tight uppercase text-slate-800 truncate">{product.name}</p>
            <p className="text-xl font-semibold text-red-600 antialiased text-end">
...
{getPrice(product._id)}
            ...
            <p className="text-xs text-gray-500 antialiased text-center">Click for more information</p>
          </div>
        </Link>
      </div>
    ))}
