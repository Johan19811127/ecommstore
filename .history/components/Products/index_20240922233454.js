...
export default function Products() {
  ...
  const getPrice = (_id) => {
    ...
    if (priceRange) {
      ...
      if (minSellingPrice < maxSellingPrice) {
        return `From R ${Number(minSellingPrice).toFixed(2)}`;
      } else {
        return `R ${Number(minSellingPriceminSellingPrice).toFixed(2)}`;
      }
    } else {
      return 'Loading...';
    }
  ...
  return (
    <div className="container mx-auto lg:w-[1200px] p-4">
      <h1 className=" text-xl md:text-3xl font-bold tet-center uppercase mb-3 mt-7 text-blue-800 text-center">OUR PRODUCTS</h1>
      <p className="text-md leading-tight text-blue-800 text-justify mb-5 md:text-center">Browse our complete collection of all products or filter our products by choosing a category below</p>
      ...
      <div className="grid  grid-cols-3 md:grid-cols-5 lg:grid-cols-6 gap-5 mt-4 mx-auto content-center items-center">
        <div className=" w-full px-3 py-3 ml-auto content-center items-center border border-gray-300 rounded-lg">
          <FaBaseballBatBall className="w-8 md:w-10 h-8 md:h-10 text-blue-800 mx-auto" />
          <p className=" text-sm md:text-md mt-3 font-semibold md:font-bold text-blue-800 uppercase text-center">SPORTSWEAR</p>
        </div>
          ...
          <div key={product._id} className="container m-1 border-b border-slate-200 shadow-md rounded-xl">
            ...
            <div className="container bg-slate-100 flex aspect-square items-center content-center rounded-t-xl border border-b-slate-200">
              {product.images && product.images[0] && (
                <Image src={product.images[0]} alt={product.name} width="400" height="400" className="dropshadow-xl rounded-t-xl "/>
              )}
            </div>
            <div className="container px-5 py-5">
              <p className="text-md font-semibold  font-sans antialiased leading-tight uppercase text-blue-800">{product.name}</p>
              <p className="text-lg font-semibold  text-blue-800 antialiased text-end">
