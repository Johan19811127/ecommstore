...
export default function cart() {
  ...
  const calculateTotal = () => {
    return cart.reduce((acc, cartItem) => {
      return acc + calculateSubtotal(cartItem.product.price, cartItem.quantity);
...
<table>
  <tbody>
    <tr>
      ...
      <th className="w-[25%] bg-blue-900 text-white px-2 py-2 text-end">Total</th>
      <th className="w-[25%] border border-gray-300">R ${Number(cartTotal).toFixed(2)}</th>
    </tr>
  </tbody>
</table>
