...
const { user, loading } = useAuth();
  ...
  const supplierInv = {
    ...
    party: supplier.name,      // Supplier ID or name
    enteredBy: user.email,     // Email of the user
    docNumber: docno,          // Document number
    transactDate: docdate,     // Date of transaction
    rows: rows                 // Invoice rows (array)
  };
  
  if (!Array.isArray(rows) || rows.length === 0) {
    alert('Rows are invalid or empty. Please check your input.');
    return;
  }
  ...
  try {
    ...
    let result;
    try {
      result = await response.json(); // Wrap in try-catch to handle non-JSON responses
    } catch (jsonError) {
 ...
 {supplier && (
  <>
          ...
          <div className="flex-row flex ">
            <div className="basis-1/3">     <LuPhone   className="w-[25px] h-[25px] mr-3"/></div>
            <div className="basis-2/3">
          <p className="block text-sm  px-3 text-gray-700  mb-2">{supplier.phone}</p>
          </div>
          </div>
          <div className="flex flex-row">
            <div className="basis-1/3">      <LuMail   className="w-[25px] h-[25px] mr-3"/>   </div>
<div className="basis-2/3">
          <p className="block text-sm  px-3 text-gray-700 " >{supplier.email}</p>
            ...
            <div className="basis-1/2">
            <p className="text-md text-end">Order Number:</p>
          </div>
          <div className="basis-1/2 ">
          <h3 className="text-xl font-bold mb-3 text-end">INV00{order}</h3>
           </div>
           </div>
            <div className="flex flex-row  text-slate-800">
            <div className="basis-1/2">
            <p className="text-md text-end">Date:</p>
          </div>
          <div className="basis-1/2 ">
           <p className="text-md text-end font-semibold">{today}</p>
           </div>
           </div>
           <div className="flex flex-row  text-slate-800">
            <div className="basis-1/2">
          ...
          <div className="basis-1/2 ">
           <p className="text-sm text-end font-semibold">{via}</p>
           </div>
           </div> 
           <div className="flex flex-row  text-slate-800">
            <div className="basis-1/2">
          ...
          <div className="basis-1/2 ">
           <p className="text-sm text-end font-semibold">{status}</p>
           </div>
           </div>  
         </div>
         </div>
        ...
        {/* Items Table */}
        <div className="min-h-[200px] lg:min-h-[250px] font-md mt-2 rounded-lg shadow-sm">
