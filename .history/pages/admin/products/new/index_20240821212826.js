...
<div key={size.size} className="px-4 py-2 bg-blue-700 rounded-lg text-white text-sm">{size.size}</div>
      ...
      <div className="container flex flex-row w-[80%] mx-auto gap-3">
         ...
         {images.map((image) => (
          <div className="basis-1/3 aspect-[4/5] rounded-lg w-[100%] items-cemter justify">
          <div className="containeer" key={supplier._id} value={supplier._id}>{supplier.name}</option>
        ))}
