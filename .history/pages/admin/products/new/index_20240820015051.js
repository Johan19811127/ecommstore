...
const ProductForm = () => {
 ...
 const [suppliers, setSuppliers] = useState([]);
  ...
  return (
   ...
   <div className="basis=1/2" > {/* Sizes */}
      <div className="mb-4">
        <label className="block text-sm font-medium text-gray-700 mx-5">Variations</label>
        {formData.sizes.length < 1 ? (
        
        <p className="text-sm px-5>Please load at least one variance for this product. </p>)
      }
        {formData.sizes.map(("> You can load as many variances as is needed</p>):(<p className="text-sm "size, index) => (
