...
const addsize = (e) => {
  setSizeLoad(true);
const allSizes = sizes.push(size);
setSizes(allSizes)
setSize({...size,[name]: ''});
  ...
  const  newImages =  async (e) =>{
    try {
      ...
      const addimages = images.push(imageUrls)
      setImages(addimages)
      console.log(images)
...
<div key={s.size} className="px-4 py-2 bg-blue-700 rounded-lg text-white text-sm">{size.size}</div>
          ))}
          </div>
      </div>
      </div>  
