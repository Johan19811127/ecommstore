...
<p className="mt-4 mb-2 uppercase text-slate-800 fopnt-bold">Item Description:</p>
          <p className=" text-gray-600">{product.description}</p>
          {product.sizes && product.sizes.length === 1 && (
            
            <><div className="border border-t-slate-500 w-[50%] mt-8 "></div><div className="mt-4  rounded-lg">

<p className="text-md">Pricing and Stock on Hand:</p>
              <p className="text-default "> Size: <span className="text-lg font-semibold ml-5 antialiased uppercase">{product.sizes[0].size}</span> </p>
              <p className="text-default "> Price: <span className="text-lg font-semibold ml-5 uppercase">R {Number(product.sizes[0].selling).toFixed(2)}</span> </p>
