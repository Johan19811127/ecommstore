      ...
      fetch(`/api/product/${id}`)
        ...
        .then(data => {
          if (data.success) {
            ...
            setySelItem(data.data.sizes[0])// Access the product data correctly
          } else {
              ...
              <div className="flex flex-row gap-3">
                {product.sizes.map((size) => (
                  <button
                    ...
                    key={size.size}
                    type="button"
                    className="selected:bg-blue-600  bg-gray-400 text-md font-semibold selected:text-white py-2 px-4 rounded-lg flex-grow-0 flex-shirk-0">
                    {size.size}
                  </button>
                ))}
              </div>
