      ...
      fetch(`/api/product/${id}`)
        ...
        .then(data => {
          if (data.success) {
            ...
            setSelItem(data.data.sizes[0]) // Set the first size as the default selected item
          } else {
          ...
          <div className="px-5">
            ...
            {product.sizes && product.sizes.length > 1 && (
              <>
               ...
               <p className="mb-1 text-blue-900 text-sm">Please choose the item you are interested in:</p>
                ...
                {product.sizes.map((size) => (
                  <button
                    ...
                    className={`text-md font-semibold py-2 px-4 rounded-lg flex-grow-0 flex-shirk-0 ${
                      selItem.size === size.size ? 'bg-blue-600 text-white' : 'bg-gray-400'
                    }`}
                  >
                    {size.size}
                  </button>
              ...
              <p className="mt-4 mb-1  text-blue-900 font-bold">Information about your selected item:</p>
                <p className="align-middle font-medium text-gray-700">
                  Size: <span className="ml-11 text-blue-900 antialiased uppercase">{selItem.size}</span>
                </p>
                <p className="align-middle font-medium text-gray-700">
                  Price: <span className="ml-9 text-blue-900 antialiased uppercase">R {Number(selItem.selling).toFixed(2)}</span>
                </p>
                <p className="align-middle font-medium text-gray-700">
                  In Stock: <span className="ml-4 text-blue-900 antialiased uppercase">{selItem.quantity}</span>
                </p>
              </>
            )}
