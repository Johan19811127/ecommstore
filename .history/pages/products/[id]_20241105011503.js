...
export default function ProductDetail() {
  ...
  const handleAddToCart = (event) => {
    ...
    if(qty > selItem.totalOnHand){
      setError("The requested quantity exceeds the available stock. Please select a number that is equal to or less than the current stock");
      return;
    }
    addToCart(product, selItem , qty); // Pass product, selected item, and quantity to cart
    alert(`${qty} ${product.name} (${selItem.size}) added to cart`);
  };
               ...
               <p className="mb-1 text-blue-800 text-sm">Refer to the <a href="/products/sizes" className="font-semibold text-lg">sizing charts </a>to ensure the perfect fit for your selection.</p>
                ...
                {selItem && (
                 ...
                 <div className="basis-1/6">
                      <p className="align-middle text-gray-700 ">Size: </p></div>
                      <div className="basis-2/6">
                      <p className="ms-3 text-xl font-semibold antialiased uppercase text-end">{selItem.size}</p></div>
                    </div>
                    <div className="flex flex-row"> <div className="basis-1/6">
                      <p className="align-middle text-gray-700">Price: </p></div>
                      <div className="basis-2/6 text-enmd">
                      <p className='ms-3 text-xl font-semibold text-red-600 text-end'>
<span className="text-sm align-super">R </span>
{selItem.selling}
                    ...
                    <div className="flex flex-row">
                    <div className="basis-1/6">
                      <p className="align-middle text-gray-700">In Stock: </p></div>
                      <div className="basis-2/6 text-end">
                      <p className="ms-3 text-xl font-semibold antialiased uppercase">{selItem.totalOnHand}</p></div>
                    </div>
                  </>
                )}
              </>
            ...
            {error && (
              <p className="text-md-text-left font-default text-red-600 my-3 mx-2">ERROR: {error}</p>
            )
          }
          </div>
        </div>
